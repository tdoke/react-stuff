...
const { Fragment, Profiler, StrictMode, Suspense } = React;
const { cloneElement, createContext, createElement, createRef, forwardRef, isValidElement } = React;
const { Component, PureComponent, memo } = React; // component creation base classes
const { Children } = React; // create and transform react elements
const { lazy  } = React; // suspense and lazy loading
const { useState, useEffect, useCallback, useContext, useDebugValue, useDeferredValue,
  useId, useImperativeHandle, useInsertionEffect, useLayoutEffect, useMemo, useReducer,
  useRef, useSyncExternalStore, useTransition
 } = React; // hooks
